...
static void HotkeyAdjustVolume(const s32 delta)
{
	...
	// EmuConfig.SPU2.OutputMuted overrides hotkeys. Inform user why volume hasn't changed.
	...
	// Volume-adjusting hotkeys override mute toggle hotkey.
...
static void HotkeyToggleMute()
{
	...
	// Attempt to toggle output muting.
	const bool muted_initial = SPU2::IsOutputMuted();
	SPU2::SetOutputMuted(!muted_initial);

	if (muted_initial)
	{
		// EmuConfig.SPU2.OutputMuted overrides hotkeys.
