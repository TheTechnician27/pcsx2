...
void SPU2::SetOutputMuted(const bool muted)
{
	// Save a restore volume if muting.
	if (!s_output_muted)
		s_unmute_volume = SPU2::GetOutputVolume();

	// User setting takes precedence.
	s_output_muted = (muted || EmuConfig.SPU2.OutputMuted);

	// Update volume to 0 if muting or restore volume if unmuting.
	SPU2::SetOutputVolume(s_output_muted ? 0 : s_unmute_volume);
}
